...
const CONFIG = {
    showMode: window.sessionStorage.getItem('stringee_show_mode')
        ? window.sessionStorage.getItem('stringee_show_mode')
        : 'min',
    bottom: 10,
    right: 10,
    arrowLeft: 155,
...
window.addEventListener('DOMContentLoaded', (event) => {
    // Livewire.emit('incoming-call', 'xxx-123-abc-246', '84374331095');
    // Livewire.emit(
...
StringeeSoftPhone.on('incomingCall', function (incomingCall) {
    ...
    console.log(
        'setCookie',
        window.sessionStorage.getItem('call_id'),
        window.sessionStorage.getItem('phone_number')
    );
});

StringeeSoftPhone.on(
    'makeOutgoingCallBtnClick',
    function (fromNumber, toNumber, callType) {
...
StringeeSoftPhone.on('signalingstate', function (state) {
    ...
    switch (state.code) {
        ...
        case STATE_CODE.BUSY:
            ...
            break;
        case STATE_CODE.END:
            const end_time_in_second =
                (new Date().getTime() -
                    parseInt(
                        window.sessionStorage.getItem('call_start_time')
                    )) /
                1000;
...
StringeeSoftPhone.on('endCallBtnClick', function (data) {
    const end_time_in_second =
        (new Date().getTime() -
            parseInt(window.sessionStorage.getItem('call_start_time'))) /
        1000;
    console.log(end_time_in_second);
...
StringeeSoftPhone.on('displayModeChange', function (event) {
    ...
    if (event === 'min') {
        window.sessionStorage.setItem('stringee_show_mode', 'min');
        StringeeSoftPhone.config({ arrowLeft: 0 });
    } else if (event === 'full') {
